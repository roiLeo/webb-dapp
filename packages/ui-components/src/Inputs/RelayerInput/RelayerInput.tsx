import { Button, ClickAwayListener, Divider, IconButton, InputBase, List, ListItemText, Popper } from '@material-ui/core';
...
const DropdownInput: React.FC<DropdownInputProps> = ({ options, onChange, value, onCustomRelayerSelect }) => {
  ...
  return (
    <div style={wrapperStyles}>
      <DropdownInput
        ...
        onCustomRelayerSelect={() => {setView(RelayerInputStatus.AddNewCustom)}}
      />
      <SpaceBox height={16} />
      <RelayerInputSection>
                  ...
                  <>
                    <ul>
                      {relayerInfo.substrate.map((i) => {
                        return <li><Typography variant={'h5'}>{i.value.beneficiary}</Typography></li>;
                      })}
                    </ul>
                  </>
                )}

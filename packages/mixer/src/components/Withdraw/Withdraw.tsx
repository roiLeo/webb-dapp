...
import { ActiveWebbRelayer, useWebContext, WebbRelayer, WithdrawState } from '@webb-dapp/react-environment';
    ...
    <WithdrawWrapper wallet={activeWallet}>
      ...
      {depositNote && (
        <AddressAndInfoSection>
          ...
          <div className='information-item'>
            ...
            <p className='value'>{fees} {depositNote.note.tokenSymbol}</p>
          </div>
          <SpaceBox height={4} />
          <div className='total-amount'>
            <p className='title'>Total Amount</p>
            <p className='value'>{Number(depositNote.note.amount) - Number(fees)} {depositNote.note.tokenSymbol}</p>
          </div>
          <SpaceBox height={8} />
          <div style={{ padding: '10px 35px' }}>
            <MixerButton
      ...
      <Modal open={showRelayerModal}>
        <RelayerModal 
          ...
          onChange={( nextRelayer: WebbRelayer | null )=>{ setRelayer(nextRelayer) }}
          onClose={()=>{ setShowRelayerModal(false) }} 
        />
      </Modal>
    </WithdrawWrapper>
  );
